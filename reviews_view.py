...
def room_reviews():
    room=Tk()
    text=Text(room)
    result=review_base.room_reviews(db)
...
def service_reviews():
    service=Tk()
    text=Text(service)
    result=review_base.service_reviews(db)
...
def breakfast_reviews():    
    breakfast=Tk()
    text=Text(breakfast)
    result=review_base.breakfast_reviews(db)
...
def add_review(root,review_name):
    ...
    if review_name=='room':
        ...
        submit_button=Button(bottomframe,text='Submit',command=lambda:review_base.update_database(db,Hotel_entry.get(),Room_Entry.get(),Rating_Entry.get(),Description_Entry.get()))
        ...
        submit_button=Button(bottomframe,text='Submit',command=lambda:review_base.update_database(db,Hotel_entry.get(),Service_Entry.get(),Rating_Entry.get(),Description_Entry.get()))
        submit_button.grid(row=4,column=1)
        ...
        submit_button=Button(bottomframe,text='Submit',command=lambda:review_base.update_database(db,Hotel_entry.get(),Breakfast_Entry.get(),Rating_Entry.get(),Description_Entry.get()))
        submit_button.grid(row=4,column=1)
