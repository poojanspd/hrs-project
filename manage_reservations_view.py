...
def add_reservation():
    ...
    Next_Button=Button(frame1,text='Next',command=lambda:[f() for f in [frame1.pack_forget,frame2.pack,reservationbase.update_database(db,Customer_Entry.get(),\
                                                                        Address_Entry.get(),Phone_Entry.get(),Email_Entry.get())]])
    ...
    Next_Button=Button(frame2,text='Next',command=lambda:[f() for f in [frame2.pack_forget,frame3.pack,reservationbase.update_database(db,Hotel_entry.get(),\
                                                                        Room_Entry.get(),CheckIn_Entry.get(),CheckOut_Entry.get())]])
    Next_Button.grid(row=4,column=1)
    ...
    Submit_Button=Button(frame3,text='Submit',command=lambda:[f() for f in [frame3.pack_forget,reservationbase.update_database(db,CNum_Entry.get(),\
